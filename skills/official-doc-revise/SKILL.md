...
若运行环境支持子代理（如 Claude Code），且 Must Fix 跨多个章节，推荐按文件所有权并行回修。

推荐分工应按当前项目的实际文件所有权和问题聚类动态确定，不得按某个项目当前的章节号写死。可按以下方式拆分：
- 按单个章节散件文件拆分，每个子代理独占 1 个或数个相邻输出文件
- 按问题类型拆分，如“正文重写”“成果/指标重写”“事实台账回补”“表图引用修复”
- 按内容类型拆分，如“背景/现状类”“研究内容/关键技术类”“成果/指标类”“台账类”
- 台账回修若独立进行，只允许修改 `facts-ledger.md`、`research-evidence.md`、`research-sources.md` 等授权文件

并行回修必须遵守：
- 每个子代理只改分配文件，不交叉改写。
- 触及五类正文章节时，子代理必须先显式加载并读取对应正文 skill，再改稿；若运行环境支持 skill 调用，仅阅读 `SKILL.md` 文件不算完成调用。
- 子代理不得新增计划外二级节，不得改全文编号方案。
- 子代理输出仅包含“已显式加载的 skill 清单 + 已改文件 + 未解决阻断项”，最终是否可装配由主控代理统一判定。

子代理任务单应复用 `skills/official-doc-core/references/subagent-task-card-template.md`，并把“独占可编辑文件”写全后再开工。
