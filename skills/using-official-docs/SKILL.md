...
若运行环境支持子代理（如 Claude Code），且命中以下任一条件，主入口可在 `research` 与 `writing` 两个阶段启用并行调度：
- 目标总字数 `>= 80,000`
- 章节数 `>= 6`
...
任务单必须直接套用 `skills/official-doc-core/references/subagent-task-card-template.md` 的字段顺序，不得自由发挥成简版 prompt。
...
6. 对每一章按关键词做路由判断
6.5. 若命中 `0.95` 的大任务条件，仅可在“调研组 / 正文章节”两个范围内派发子代理并行执行；每个子代理必须有独占写入范围
