...
若运行环境支持子代理（如 Claude Code），且命中以下任一条件，主入口应启用并行调度：
- 目标总字数 `>= 80,000`
- 章节数 `>= 6`
- 激活调研组 `>= 3` 且预计有效来源总量 `>= 30`
- 已进入 `review -> revise` 循环且 `formal-draft` 超过 `50,000` 字
...
推荐派发方式：
- 调研阶段：按 `BG / RC / IN / TA / TI` 分组并行。
- 写作阶段：按章节文件并行，每个子代理独占一个 `workspace/outputs/<project-slug>/chXX-*.md`。
- 复核阶段：结构风格、事实来源、深度闭环三类并行复核。
- 回修阶段：按 Must Fix 的文件所有权并行修复。
...
任务单建议直接套用 `skills/official-doc-core/references/subagent-task-card-template.md`，不要自由发挥字段。

若运行环境支持 skill 显式调用（如 Claude Code），主入口派发子代理时必须把“必须显式加载的 skill”写成精确 skill 名，子代理开工前必须真的加载这些 skill。仅把对应 `SKILL.md` 路径列入“必读规则文件”不算完成调用。
...
若主入口发现子代理返回结果时没有声明“已显式加载的 skill 清单与顺序”，或实际执行痕迹显示未加载对应 skill，应视为该次派发无效，必须中断并按正确 skill 任务单重派，不能直接合并该结果。
...
6. 对每一章按关键词做路由判断
6.5. 若命中 `0.95` 的大任务条件，按“调研组/章节/问题清单”派发子代理并行执行；每个子代理必须有独占写入范围
   - 写作子代理的任务单必须写明“必须显式加载的 skill”字段；不能只写“按对应专项 skill 执行”
   - 写作子代理要加载哪些 skill，必须按当前任务命中的内容类型动态判定，不能按固定章节号、固定文件名或当前项目结构写死
   - 在支持 skill 调用的环境中，子代理必须先显式加载 skill，再开始读写文件；若未加载，主入口必须视为派发失败并重派
