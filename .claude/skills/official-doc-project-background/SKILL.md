...
写本章前，必须先做“证据抽取”而不是直接扩写。每个二级节至少从 `research-evidence.md`、`research-notes.md`、`facts-ledger.md` 中整理出 3 个可直接消费的证据点，其中至少包含：
- 1 个政策/官方依据
- 1 个行业/单位场景或现状依据
- 1 个差距、影响或项目切入依据

正文不得直接写出 `F-001`、`BG-S03` 这类内部编号，但段落中的政策要求、场景问题、差距判断和项目必要性必须都能回溯到这些证据点。若一个二级节写完后只有宏观形势和价值判断，看不出具体政策、场景问题、影响后果和项目切入，视为“调研未消费”，必须退回重写或补调研。

背景类文字必须收口，不能把“战略支撑、产业链安全、核心能力建设、生态体系构建、高质量发展”连续堆在一段里。每段优先保留 `场景对象 + 现有问题 + 影响后果 + 项目切入`，将纯价值判断压缩到结尾 1 句。若某段删除这些价值词后几乎不损失信息，说明该段过泛过满，必须压缩重写。

扩写本章时，不得新增 `00-section-plan.md` 未登记的二级节，也不得用连续 `### 11.`、`### 12.`、`### 13.` 之类流水号标题堆篇幅。字数不足时，只能在既定二级节内补充政策依据、国内外差距、场景问题、影响后果和项目切入。

若本章由子代理执行（如 Claude Code 并行任务），该子代理的首个动作必须是显式加载 `official-doc-project-background`，并在开始任何读写前先回报“已显式加载：official-doc-project-background”；未完成这一步，不得读取台账或写稿。该子代理也必须完整遵守本 skill 与 `attachment-writing-patterns.md` 的硬规则；不得因“并行补字数”放宽句式、编号、事实核验或小节边界要求。
...
只能撰写 `00-section-plan.md` 为本章登记的二级节。若用户只要求 `行业背景` 和 `立项意义`，不得自行追加 `项目目标与预期成果`、`项目组织实施` 等二级节；需要扩写时，应在既有二级节内补充现状、差距、影响和项目切入。
