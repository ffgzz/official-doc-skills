...
- 若用户给出章节字数要求，本次正式稿每章最低字数、当前统计字数与偏差是多少
...
- 但“唯一数据源”只限于 `00-section-plan.md` 已登记的章节正文、表格和图示；`research-mapping.md`、抓取说明、检索映射、notes、过程日志等不属于装配对象
...
- 如果用户给出了章节字数硬约束且任一章节未达到最低字数，不得进入装配
- 如果正文仍明显保留列表化、加粗小标签或机械过渡词，不得进入装配
- 如果正文或 formal-draft 仍保留 `【来源：...】`、`来源：原始资料`、`BG-1-01 / RC-1-01` 这类内部来源标记，不得进入装配
- 如果 `workspace/outputs/<project-slug>/` 中混入 `research-mapping.md`、检索执行概况、来源分组映射或其他内部调研说明文件，必须先清理或移回 `workspace/plan/<project-slug>/`，不得装配
...
- 若任一 `.mmd` 图源未通过 Mermaid 语法校验，或装配稿中已经出现 `Syntax error in text`、Mermaid 解析失败占位图、明显残缺的图示说明，不得装配
- 若正式稿仍包含未核验的高风险具体事实，不能用 assemble 掩盖该问题，必须回到 revise / review 修复

## 装配阶段不派子代理

`official-doc-assemble` 必须由主控代理直接执行，不得派发装配预检子代理。装配依赖当前正式稿、表图落位、最新 review 结论和 assembly-notes 的实时统计，拆给子代理后容易产生统计滞后和结论分裂。

因此：
- 不得生成装配预检子代理
- 不得生成任何 `assemble-worker`、`precheck-worker` 一类中间报告
- 必须由主控代理直接完成装配前判定、正式装配和装配后 final review 触发
...
- 若任一章节字数未达到 `00-section-plan.md` 的最低字数，暂不可装配；必须回到 revise 对该章定向补足后重新 review
- 若当前 `assembly-notes` 的字数、比例、`【待补】` 数与当前 `formal-draft` 不一致，也不得判为正式稿
- 若表图尚未装配到对应章节附近、正文仍保留引图引表占位，暂不可装配
- 若 formal-draft 中出现任何非 `00-section-plan.md` 登记章节的内部说明，如 `检索与资料映射说明`、`调研执行概况`、`来源分组与章节映射`，暂不可装配
- 若 `agent-skill-trace.md` 为空且本轮属于正常多章节写作，也暂不可装配
...
3.5. 检查是否混入内部调研说明或来源标记，包括 `检索与资料映射说明`、`【来源：...】`、`来源：原始资料`
...
- 若某文件虽然位于 `workspace/outputs/<project-slug>/`，但不属于 `00-section-plan.md` 已登记章节，例如 `research-mapping.md`、过程说明、来源映射说明，必须完全排除，不得因为“也在 outputs 目录中”就被顺手装入
...
- 若用户给出了总字数或章节字数要求，还必须在 `assembly-notes` 和 `progress.md` 中记录“目标总字数 / 当前总字数 / 是否达标”以及“每章最低字数 / 当前字数 / 是否达标”
