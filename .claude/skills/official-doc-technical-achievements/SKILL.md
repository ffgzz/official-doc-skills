...
写成果前，必须先从 `research-evidence.md`、`research-notes.md`、`facts-ledger.md` 中为每项成果抽出至少 2 个证据点：1 个用于回指来源研究内容，1 个用于校准成果形态、应用场景或验收方式。不能从研究内容自然推导出来的条目，不得写成成果。

成果正文不得裸露内部来源编号，但每项成果的命名、功能边界、交付形态和验收方式都必须能回溯到证据卡或事实台账。若某项成果写完后只剩“提升能力、支撑应用、形成体系”，说明调研和研究内容没有被消费，必须改写为可交付实体。
...
若本章由子代理执行（如 Claude Code 并行任务），该子代理的首个动作必须是显式加载 `official-doc-technical-achievements`，并在开始任何读写前先回报“已显式加载：official-doc-technical-achievements”；未完成这一步，不得读取台账或写稿。该子代理也必须完整遵守本 skill 与 `attachment-writing-patterns.md` 的硬规则；每项成果都要给出对应研究内容、交付形态和验收方式，不得只列愿景性结果。
